...
- `docker-compose.yml` (at root): Provides shared services for testing, notably the `lenz` ApertureDB container and the `ca` cert generator.
...
2. Starts `lenz` (ApertureDB) locally.
...
- **Environment Variables:** Workflows expect specific ApertureDB connection variables: `DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASS`, and `CA_CERT`. During local tests, `DB_HOST` resolves to the `lenz` service.
